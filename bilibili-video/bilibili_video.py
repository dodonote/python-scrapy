...
import re
import json
import pymysql as MySQLdb #重命名导入模组在当前模组中的名
...
def spider(url):
    ...
    # print html
    # exit()
    selector = etree.HTML(html.text) #已lxml解析html
    # print selector
    content = selector.xpath("//html") #从html处开始
    # print content
    # exit
    for each in content:
        ...
        if title:
            ...
            if cid_html_1 or cid_html_2:
                ...
                for video_each in video_selector:
                    ...
                    jsDict = json.loads(jsoncontent) #将字符串转python对象，json.dumps将对象转字符串
                    if jsDict['code'] == 0:
                        jsData = jsDict['data']
                        jsPages = jsData['page']
                        ...
                        try:
                            ...
                            print "Succeed: av" + str(av) #str将对象转换为一种string的格式，适应阅读格式
